...
from src.metrics import average_precision_clevr, accuracy
...
model = SlotAttentionReasoning(resolution, 
...
def training_step(model, optimizer, epoch, opt):
    idxs = []
    total_loss = 0; reasoning_loss = 0; recon_loss = 0; property_loss = 0; qloss = 0;
    for ibatch, sample in tqdm(enumerate(train_dataloader)):
        global_step = epoch * train_epoch_size + ibatch
        image = sample['image'].to(device)
        properties = sample['properties'].to(device)
        labels = sample['label'].to(device)

        recon_combined, predictions, logits, recons, masks, slots, cbidxs, qerror, perplexity = model(image, 
                                                        epoch=epoch, 
                                                        batch=ibatch)
        property_error = hungarian_huber_loss(predictions, properties)
        reasoning_error = F.cross_entropy_loss(logits, labels)
        recon_error = ((recon_combined - labels)**2).mean()

        loss = recon_error + reasoning_error + property_error + qerror

        total_loss += loss.item()
        reasoning_loss += reasoning_error.item()
        ...
        with torch.no_grad():
            if ibatch % opt.log_interval == 0:            
                ...
                writer.add_scalar('TRAIN/recon_loss', recon_error.item(), global_step)
                ...
                writer.add_scalar('TRAIN/reasoning_loss', reasoning_error.item(), global_step)
...
def validation_step(model, optimizer, epoch, opt):
    ...
    for ibatch, sample in tqdm(enumerate(train_dataloader)):
        ...
        if ibatch % opt.log_interval == 0:            
            ...
            writer.add_scalar('VALID/recon_loss', recon_error.item(), global_step)
            ...
            writer.add_scalar('VALID/reasoning_loss', reasoning_error.item(), global_step)
    ...
    return_stats = {'total_loss': total_loss*1.0/val_epoch_size,
                        ...
                        'reasoning_loss': reasoning_loss*1.0/val_epoch_size,
                        'unique_idxs': torch.unique(idxs).cpu().numpy()}
    return return_stats
...
for epoch in range(opt.num_epochs):
    ...
    with torch.no_grad():
        ...
        if not epoch  % every_nepoch:
            ...
            acc = accuracy(test_dataloader, model, 25)
            writer.add_scalar('VALID/ReasoningAcc', acc, epoch//every_nepoch)
