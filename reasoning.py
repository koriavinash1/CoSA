...
Roptimizer = optim.Adam(Rmodel.parameters(), lr=opt.learning_rate)
...
def training_step(AEmodel, Rmodel, AEoptimizer, Roptimizer, epoch, opt):
    idxs = []
    ...
    for ibatch, sample in tqdm(enumerate(train_dataloader)):
        ...
        # =================
        logits = Rmodel(slots.detach(), cbidxs, 
...
@torch.no_grad()
def validation_step(AEmodel, Rmodel, AEoptimizer, Roptimizer, epoch, opt):
    idxs = []
    total_loss = 0; reasoning_loss = 0; recon_loss = 0; property_loss = 0; quant_loss = 0; overlap_loss = 0
    for ibatch, sample in tqdm(enumerate(train_dataloader)):
        global_step = epoch * train_epoch_size + ibatch
...
for epoch in range(opt.num_epochs):
    ...
    training_stats = training_step(AEmodel, Rmodel, AEoptimizer, Roptimizer, epoch, opt)
    ...
    validation_stats = validation_step(AEmodel, Rmodel, AEoptimizer, Roptimizer, epoch, opt)
